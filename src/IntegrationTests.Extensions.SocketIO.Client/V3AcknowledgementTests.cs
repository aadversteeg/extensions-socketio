...
public class V3AcknowledgementTests : V3IntegrationTestBase
{
    ...
    [Fact(DisplayName = "V3A-003: Ack with string parameter — echoed back via 1:ack")]
    public async Task V3A003()
    {
        if (ShouldSkip) return;
    ...
    [Fact(DisplayName = "V3A-004: Server-side ack — client sends ack data, server echoes result")]
    public async Task V3A004()
    {
        if (ShouldSkip) return;
