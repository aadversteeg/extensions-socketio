...
/// </summary>
public class SocketIOClient : ISocketIOClient, IInternalSocketIOClient
{
    ...
    private readonly object _ackHandlerLock = new object();
    ...
    private readonly Dictionary<int, Func<IDataMessage, Task>> _ackHandlers = new Dictionary<int, Func<IDataMessage, Task>>();
    private readonly Dictionary<string, Func<IEventContext, Task>> _eventHandlers = new Dictionary<string, Func<IEventContext, Task>>();
    private readonly HashSet<string> _onceEvents = new HashSet<string>();
    private readonly List<Func<string, IEventContext, Task>> _onAnyHandlers = new List<Func<string, IEventContext, Task>>();

    private TaskCompletionSource<bool>? _sessionCompletionSource;
    private TaskCompletionSource<Exception?>? _connCompletionSource;

    /// <summary>
    /// Initializes a new instance of the <see cref="SocketIOClient"/> class.
    /// </summary>
    public SocketIOClient(Uri uri, SocketIOClientOptions options, Action<IServiceCollection>? configure = null)
    {
        ServerUri = uri;
        Options = options;
        var services = new ServiceCollection();
        _serviceProvider = ServiceCollectionExtensions.BuildServiceProvider(services, configure);
        _logger = _serviceProvider.GetRequiredService<ILogger<SocketIOClient>>();
    ...
    /// Initializes a new instance of the <see cref="SocketIOClient"/> class.
    /// </summary>
    public SocketIOClient(Uri uri, Action<IServiceCollection> configure) : this(uri, new SocketIOClientOptions(), configure)
    {
    }
    ...
    /// Initializes a new instance of the <see cref="SocketIOClient"/> class.
    /// </summary>
    public SocketIOClient(Uri uri) : this(uri, new SocketIOClientOptions())
    {
    }

    int IInternalSocketIOClient.PacketId => _packetId;
    ...
    int IInternalSocketIOClient.AckHandlerCount => _ackHandlers.Count;

    /// <inheritdoc />
    public SocketIOClientOptions Options { get; }

    /// <summary>
    /// Occurs when a reconnection attempt fails.
    /// </summary>
    public event EventHandler<Exception>? OnReconnectError;

    /// <summary>
    /// Occurs when a ping is sent.
    /// </summary>
    ...
    {
        await EmitAsync(eventName, Enumerable.Empty<object>(), cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        await EmitAsync(eventName, Enumerable.Empty<object>(), ack, cancellationToken).ConfigureAwait(false);
    }
    ...
    async Task IInternalSocketIOClient.SendAckDataAsync(int packetId, IEnumerable<object> data)
    {
